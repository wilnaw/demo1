(function(factory) { 'use strict';
    if (typeof define === 'function' && define.amd) { define(['jquery'], factory) } else if (typeof module !== 'undefined' && module.exports) { module.exports = factory(require('jquery')) } else { factory(jQuery) } })(function($) {
    ...
    var _rows = function(elements) {
        ...
        $elements.each(function() {
            ...
            if (lastRow === null) { rows.push($that) } else {
                if (Math.floor(Math.abs(lastTop - top)) <= tolerance) { rows[rows.length - 1] = lastRow.add($that) } else { rows.push($that) } }
            lastTop = top
        });
        return rows
    };
    ...
    matchHeight._apply = function(elements, options) {
        ...
        $.each(rows, function(key, row) {
            var $row = $(row),
                targetHeight = 0;
            if (!opts.target) {
                if (opts.byRow && $row.length <= 1) { $row.css(opts.property, '');
                    return }
                $row.each(function() {
                    var $that = $(this),
                    ...
                    css[opts.property] = '';
                    $that.css(css);
                    if ($that.outerHeight(!1) > targetHeight) { targetHeight = $that.outerHeight(!1) }
        ...
        if (matchHeight._maintainScroll) { $(window).scrollTop((scrollTop / htmlHeight) * $('html').outerHeight(!0)) }
        return this
    };
    ...
    $(window).bind('resize orientationchange', function(event) { matchHeight._update(!0, event) })
})
